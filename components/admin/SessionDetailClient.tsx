      ...
      <div className="grid gap-6">
        ...
        <PaperCard shadow="soft" className="p-5">
          <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
            <div>
              <h2 className="font-[family-name:var(--font-serif)] font-bold text-[16px] flex items-center gap-2">
                <MessageCircle size={16} className="text-stamp-blue" />
                Résumé conversation
              </h2>
              <p className="font-[family-name:var(--font-serif)] text-[13px] text-ink-faded mt-1">
                Métadonnées uniquement. Le contenu des messages et médias reste masqué.
              </p>
            </div>
            {conversationSummary.kind === "ok" && (
              <Badge
                tone={conversationSummary.summary.status === "active" ? "blue" : "neutral"}
                dot
              >
                {conversationSummary.summary.status === "active" ? "chat actif" : "aucun chat"}
              </Badge>
            )}
          </div>

          {conversationSummary.kind === "ok" ? (
            conversationSummary.summary.status === "not_started" ? (
              <div className="rounded-[4px] border border-dashed border-[var(--ink-line)] bg-paper-warm/60 p-4">
                <p className="font-[family-name:var(--font-serif)] text-[14px] text-ink-faded">
                  Aucun chat n'a encore été ouvert pour cette session.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                <div className="bg-paper-warm rounded-[4px] p-3">
                  <div className="font-[family-name:var(--font-serif)] font-bold text-[22px]">
                    {conversationSummary.summary.messageCount}
                  </div>
                ...
                <div className="bg-paper-warm rounded-[4px] p-3">
                  <div className="font-[family-name:var(--font-serif)] font-bold text-[22px]">
                    {conversationSummary.summary.participantCount}
                  </div>
                  <div className="font-[family-name:var(--font-type)] text-[9px] uppercase tracking-[0.1em] text-ink-faded">
                    participants
                  </div>
                ...
                <div className="bg-paper-warm rounded-[4px] p-3">
                  <div className="font-[family-name:var(--font-serif)] font-bold text-[22px]">
                    {conversationSummary.summary.reportCount}
                  </div>
                  <div className="font-[family-name:var(--font-type)] text-[9px] uppercase tracking-[0.1em] text-ink-faded">
                    signalements liés
                  </div>
                ...
                <div className="bg-paper-warm rounded-[4px] p-3">
                  <div className="font-[family-name:var(--font-serif)] font-bold text-[13px]">
                    {formatDateTime(conversationSummary.summary.firstMessageAt)}
                  </div>
                  <div className="font-[family-name:var(--font-type)] text-[9px] uppercase tracking-[0.1em] text-ink-faded mt-1">
                    premier message
                  </div>
                </div>
                <div className="bg-paper-warm rounded-[4px] p-3">
                  <div className="font-[family-name:var(--font-serif)] font-bold text-[13px]">
                    {formatDateTime(conversationSummary.summary.lastMessageAt)}
                  </div>
                  <div className="font-[family-name:var(--font-type)] text-[9px] uppercase tracking-[0.1em] text-ink-faded mt-1">
                    dernier message
                  </div>
            ...
            <div className="flex items-start gap-2 rounded-[4px] border border-stamp-red/30 bg-stamp-red/8 p-4">
              ...
              <div>
                <p className="font-[family-name:var(--font-serif)] text-[14px] font-semibold">
                  Résumé conversation indisponible
                </p>
                <p className="font-[family-name:var(--font-serif)] text-[13px] text-ink-faded mt-1">
                  {conversationSummary.kind === "unauthorized"
                    ? "Votre compte staff n'a pas l'autorisation de voir ces métadonnées."
                    : conversationSummary.kind === "unauthenticated"
                      ? "Reconnectez-vous pour charger les métadonnées de conversation."
                      : conversationSummary.kind === "not_found"
                        ? "La session liée à ce résumé est introuvable côté backend."
